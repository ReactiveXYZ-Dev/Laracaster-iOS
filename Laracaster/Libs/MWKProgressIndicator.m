...
- (id)init
{
    ...
    [_titleLabel setFont:[UIFont fontWithName:@"AppleSDGothicNeo-Bold" size:20]];
...
- (void)show
{
    ...
    if (CGRectGetMinY(self.frame) == 0.0) return;
...
{
    [self updateMessage:message type:MWKProgressMessageUpdateTypeText];
}
...
- (void)updateMessage:(NSString *)message type:(MWKProgressMessageUpdateType)type
{
    dispatch_async(dispatch_get_main_queue(), ^
    {
        ...
        {
            [self updateTitleMessage:message];
        }
...
-(void)updateTitleMessage:(NSString *)message{
...
{
    [self showWithColor:[UIColor redColor] duration:2 message:errorMessage];
}

- (void)showSuccessMessage:(NSString *)successMessage
{
    [self showWithColor:[UIColor greenColor] duration:2 message:successMessage];
}
                           ...
                           dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(hideDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^
                            {
                                    ...
                                    [self updateTitleMessage:@"Loading"];
